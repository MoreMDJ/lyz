...
public interface TdSalesDetailRepo extends PagingAndSortingRepository<TdSalesDetail, Long>, JpaSpecificationExecutor<TdSalesDetail> {
	...
	@Query(value = "select og.id id,o.diy_site_name diy_site_name,o.main_order_number main_order_number,o.order_number order_number,o.order_time order_time, "
			+ "di.begin_dt sale_time,o.status_id status_id,su.real_name seller_real_name,u.real_name real_name,IFNULL(o.real_user_username,o.username) "
			...
			+ " where o.order_time>=?1 and o.order_time<=?2 and o.status_id not in(1,2,7,8) and o.city like ?3 and o.diy_site_code like ?4 and o.diy_site_id in ?5 ;",nativeQuery = true)
